...
static void publish_pressure_sensor_depth(float pressure_depth_m)
{
    std_msgs__msg__Float32 pressure_depth_msg;
    pressure_depth_msg.data = pressure_depth_m;

    rcl_ret_t ret = rcl_publish(&pressure_sensor_depth_publisher, &pressure_depth_msg, NULL);
    if (ret != RCL_RET_OK)
    {
        char msg[128];
...
{
    float pressure_depth_m = 0.0f;
    float latest_pressure_depth_m = 0.0f;
    bool has_sample = false;

    while (osMessageQueueGet(pressure_sensor_depth_queue_handle, &pressure_depth_m, NULL, 0U) == osOK) {
        latest_pressure_depth_m = pressure_depth_m;
        has_sample = true;
    }

    if (has_sample) {
        publish_pressure_sensor_depth(latest_pressure_depth_m);
    }
}
