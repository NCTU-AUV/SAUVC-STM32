#include "motor_pwm_controller_node/motor_pwm_controller_node.h"

#include "motor_pwm_controller_node/publish_is_pwm_output_on.h"
#include "motor_pwm_controller_node/subscribe_set_pwm_output_on.h"
#include "motor_pwm_controller_node/publish_pwm_output_signal_value.h"
#include "motor_pwm_controller_node/subscribe_set_pwm_output_signal_value.h"

#include <std_msgs/msg/bool.h>


const unsigned int MOTOR_PWM_CONTROLLER_NUM_HANDLES = 1 + 8 + 8;


static rcl_node_t motor_pwm_controller_node;
static rcl_timer_t motor_pwm_controller_timer;


static void motor_pwm_controller_timer_callback(rcl_timer_t *, int64_t)
{
    publish_is_pwm_output_on_only_when_updated(MOTOR0);
    publish_is_pwm_output_on_only_when_updated(MOTOR1);
    publish_is_pwm_output_on_only_when_updated(MOTOR2);
    publish_is_pwm_output_on_only_when_updated(MOTOR3);
    publish_is_pwm_output_on_only_when_updated(MOTOR4);
    publish_is_pwm_output_on_only_when_updated(MOTOR5);
    publish_is_pwm_output_on_only_when_updated(MOTOR6);
    publish_is_pwm_output_on_only_when_updated(MOTOR7);

    publish_pwm_output_signal_value_only_when_updated(MOTOR0);
    publish_pwm_output_signal_value_only_when_updated(MOTOR1);
    publish_pwm_output_signal_value_only_when_updated(MOTOR2);
    publish_pwm_output_signal_value_only_when_updated(MOTOR3);
    publish_pwm_output_signal_value_only_when_updated(MOTOR4);
    publish_pwm_output_signal_value_only_when_updated(MOTOR5);
    publish_pwm_output_signal_value_only_when_updated(MOTOR6);
    publish_pwm_output_signal_value_only_when_updated(MOTOR7);
}


static void initialize_motor_pwm_controller_timer(rclc_support_t *support, rclc_executor_t *executor)
{
    motor_pwm_controller_timer = rcl_get_zero_initialized_timer();
    const unsigned int motor_pwm_controller_timer_timeout = 100;
    rcl_ret_t rc = rclc_timer_init_default(
        &motor_pwm_controller_timer,
        support,
        RCL_MS_TO_NS(motor_pwm_controller_timer_timeout),
        motor_pwm_controller_timer_callback
    );
    if (rc != RCL_RET_OK) {
        printf("Error in rcl_timer_init_default.\n");
    } else {
        printf("Created timer with timeout %d ms.\n", motor_pwm_controller_timer_timeout);
    }

    rclc_executor_add_timer(executor, &motor_pwm_controller_timer);
    if (rc != RCL_RET_OK) {
        printf("Error in rclc_executor_add_timer.\n");
    }
}


void initialize_motor_pwm_controller_node(rclc_support_t *support, rclc_executor_t *executor)
{
    rclc_node_init_default(&motor_pwm_controller_node, "motor_pwm_controller_node", "orca_auv", support);

    initialize_is_pwm_output_on_publisher_for_all_motors(&motor_pwm_controller_node);
    initialize_pwm_output_signal_value_publisher_for_all_motors(&motor_pwm_controller_node);
    initialize_motor_pwm_controller_timer(support, executor);

    initialize_set_pwm_output_on_subscriber_for_all_motors(&motor_pwm_controller_node, executor);
    initialize_set_pwm_output_signal_value_subscriber_for_all_motors(&motor_pwm_controller_node, executor);

    initialize_previous_is_pwm_output_on_state_for_all_motors();
    initialize_previous_pwm_output_signal_value_for_all_motors();
}
